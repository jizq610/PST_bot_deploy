...
def get_kickoff_message_for_phase(phase: str) -> str:
    return {
        "AC": "Now let's look at what happens before and after the behavior. What usually happens right before it?",
        "STRATEGY": "Now let's think about strategies for the behavior. What could be changed before the behavior?",
    }[phase]
    ...
    if st.button(
        ...
        try:
            uploaded_file = upload_excel_to_drive(
                excel_data,
                f"test_3agent_modela_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
            )
